...
batch_client.pool.get(poolid).then((poolinfo) => {
        ...
        {
            context.log('Auto Scale is enabled.');

            var autoScaleProperties ={
                autoScaleFormula: autoScaleFormula(maxNodes),
                autoScaleEvaluationInterval: moment.duration(5, 'minutes')
            };

            //enable if first and set to current.
            batch_client.pool.enableAutoScale(poolid, autoScaleProperties).then((result) =>{
...
function autoScaleFormula(maxNodes){
